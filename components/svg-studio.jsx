...
function getBounds(element) {
  const applyTransform = (bounds, transform) => {
...
function transformStyle(element) {
  if (!element.transform) return void 0;
  const parts = [];
...
function SvgStudio() {
  ...
  const [canvasWidthInput, setCanvasWidthInput] = useState(() => doc.width.toString());
  const [canvasHeightInput, setCanvasHeightInput] = useState(() => doc.height.toString());
  const [zoom, setZoom] = useState(1);
  const dragLastPointRef = useRef(null);
  const dragAppliedPointRef = useRef(null);
  ...
  const changeZoom = (delta) => setZoom((prev) => clampZoom(prev + delta));
  ...
  useEffect(() => {
    ...
    const contentHeight = doc.height || 800;
  ...
  const beginLineHandleDrag = (line, mode, event) => {
  ...
  useEffect(() => {
    const applyDragFrame = () => {
      ...
      if (dx === 0 && dy === 0) return;
      const ids = selectedIdsRef.current.size > 0 ? Array.from(selectedIdsRef.current) : selectedIdRef.current ? [selectedIdRef.current] : [];
    ...
    const handleMove = (event) => {
      ...
      if (isMarqueeSelecting) {
        ...
        elements.forEach((el) => {
          ...
          const bounds = getElementBoundsWithRef(el);
          if (!bounds) return;
          const intersects = bounds.x - margin <= rect.x + rect.width && bounds.x + bounds.width + margin >= rect.x && bounds.y - margin <= rect.y + rect.height && bounds.y + bounds.height + margin >= rect.y;
          if (intersects) {
            next.add(el.id);
          }
        });
  ...
  useEffect(() => {
    const handleMove = (event) => {
      ...
      const snap = findSnapAnchor(pointer);
      updateElement(
        drag.id,
        (el) => {
          if (el.type !== "line") return el;
  ...
  const handleAnchorDown = (event, anchor, elementId) => {
    event.stopPropagation();
    event.preventDefault();
    commitSnapshot();
  ...
  const handleCanvasPointerDown = (event) => {
    ...
    const drawMode = tool === "rect" || tool === "ellipse" || tool === "line" ? tool : "rect";
    if (drawMode === "line") {
      const snapStart = findSnapAnchor(point);
  ...
  const handleCanvasPointerMove = (event) => {
    ...
    const point = canvasPoint(event);
    if (draft.mode === "line") {
      const snapEnd = findSnapAnchor(point);
      setDraft(
        (prev) => prev ? {
          ...prev,
  ...
  const handleCanvasPointerUp = () => {
    ...
    const tooSmall = mode === "line" && Math.hypot(width, height) < 6 || mode !== "line" && normalized.width < 4 && normalized.height < 4;
    if (tooSmall) {
      setDraft(null);
      setTool("select");
      return;
  ...
  const handleElementPointerDown = (event, element) => {
    ...
    if (event.shiftKey || event.metaKey || event.ctrlKey) {
      ...
      if (alreadySelected && selectedIds.size > 1) {
        ...
        setSelectedIds(/* @__PURE__ */ new Set([element.id]));
  ...
  const alignSelection = (direction) => {
  ...
  const aiAutoConnect = () => {
    ...
    const list = ids.map((id) => elements.find((el) => el.id === id && el.visible !== false)).filter(Boolean);
  ...
  const aiDistributeHorizontally = () => {
    ...
    const boundsList = list.map((el) => ({ el, bounds: getElementBoundsWithRef(el) })).filter((item) => item.bounds);
  ...
  return <div className="flex h-full w-full gap-3">
    ...
    return <button
      ...
      className={cn(
        ...
        active ? "bg-slate-900 text-white shadow-sm" : "bg-slate-50 text-slate-700 hover:bg-slate-100"
      )}
    >
  ...
  >
    ...
    className={cn(
      ...
      snapEnabled ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-100"
    )}
  >
  ...
  >
    ...
    style={{
      ...
      // Assuming grid size scales? Or just fixed?
  ...
  >
    ...
    const commonProps = {
      ...
      className: cn(
        "cursor-default",
        (selectedIds.has(element.id) || selectedId === element.id) && "outline-none ring-2 ring-offset-2 ring-blue-500/50"
      )
    };
    switch (element.type) {
      case "rect":
    ...
    pointerEvents="none"
  />}
                                {marqueeRect && isMarqueeSelecting && <rect
    x={marqueeRect.x}
    y={marqueeRect.y}
  ...
  />}
    ...
    pointerEvents="none"
  />)}
                                {selectedElement?.type === "line" && selectedElement.visible !== false ? <LineHandles
    line={selectedElement}
    onHandleDown={beginLineHandleDrag}
  /> : measuredBounds && selectedElement && selectedElement.visible !== false && <ResizeHandles
  ...
  />}
    ...
    return anchors.map((anchor, i) => <circle
      key={i}
      cx={anchor.x}
      cy={anchor.y}
      r={4}
  ...
  />
                        ...
                        {selectedElement.type === "text" && <div className="space-y-2">
  ...
  />
                    ...
                    </div> : <div className="rounded-md border border-dashed border-slate-200 bg-slate-50 px-3 py-4 text-xs text-slate-500">
                ...
                <div className="mt-4 border-t border-slate-100 pt-3">
                    ...
                    {history.length === 0 ? <p className="text-xs text-slate-500">
                            暂无快照，AI 生成或导入时会自动记录。
...
function GridPattern() {
  const size = 20;
  return <svg className="h-full w-full" aria-hidden>
            <defs>
                <pattern
    id="grid"
    x="0"
...
function LineHandles({
  ...
  const handle = (mode) => (event) => onHandleDown(line, mode, event);
  return <>
            <circle
    cx={line.x1}
    cy={line.y1}
