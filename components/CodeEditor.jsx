  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
        ...
        <div className="flex gap-2">
          {!isEditing && (
            <button
              ...
              disabled={!code}
              title="Manually edit the JSON code"
            >
              Edit
            </button>
          )}
        ...
        {code ? (
          isEditing ? (
            <textarea
              ...
              className="w-full h-full text-sm font-mono bg-white p-4 rounded-lg border-2 border-blue-300 focus:outline-none focus:border-blue-500 resize-none"
              spellCheck={false}
            />
          ) : (
      ...
      <div className="p-4 border-t border-gray-200 space-y-2">
        ...
        {code && !isEditing && (
          <p className="text-xs text-gray-500 text-center">{code.length} characters</p>
        )}
        {isEditing && (
          <p className="text-xs text-blue-600 text-center">
            💡 Tip: Fix any JSON syntax errors, then click Save
          </p>
        )}
      </div>
    </div>
