...
const CONVERT_ENDPOINT = process.env.NEXT_PUBLIC_DRAWIO_CONVERT_URL || "https://convert.diagrams.net/export";
...
async function renderDiagramPng(xml) {
  if (!xml) return null;
  try {
...
async function POST(req) {
  try {
    ...
    const buffer = Buffer.isBuffer(rawOutput) ? rawOutput : Buffer.from(rawOutput);
    const baseName = (payload.topic || "FlowPilot").replace(/[^a-zA-Z0-9\u4e00-\u9fa5-_]+/g, "-").slice(0, 60) || "FlowPilot";
    const preferredFileName = `${baseName}.pptx`;
    const asciiFallbackBase = baseName.replace(/[^a-zA-Z0-9-_]+/g, "") || "FlowPilot";
    ...
    const message = error instanceof Error ? error.message : "\u5BFC\u51FA PPTX \u5931\u8D25\u3002";
