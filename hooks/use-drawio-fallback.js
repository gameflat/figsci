...
/**
 * @typedef {Object} UseDrawioFallbackOptions
 * @property {string} [primaryUrl] - 主DrawIO URL（从环境变量读取）
 * @property {string} [fallbackUrl] - 备用DrawIO URL
 * @property {number} [timeout] - 加载超时时间（毫秒），默认15秒
 * @property {boolean} [enableFallback] - 是否启用自动降级，默认true
 * @property {(from: string, to: string) => void} [onFallback] - 当发生降级时的回调
 */

/**
 * @typedef {Object} UseDrawioFallbackReturn
 * @property {string} currentUrl - 当前使用的DrawIO URL
 * @property {boolean} isLoading - 是否正在加载
 * @property {string | null} error - 错误信息
 * @property {boolean} isFallback - 是否已降级到备用URL
 * @property {() => void} retryPrimary - 手动重试主URL
 * @property {() => void} handleLoad - 手动触发加载成功
 * @property {(errorMessage?: string) => void} handleError - 手动触发加载失败
 */
 ...
 * 4. 提供加载状态和错误信息
 * 
 * @param {UseDrawioFallbackOptions} [options={}]
 * @returns {UseDrawioFallbackReturn}
 */
export function useDrawioFallback(options = {}) {
    const {
