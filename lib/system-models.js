...
export function getStaticSystemModelList() {
    const envValue = process.env.SYSTEM_MODELS;
    if (!envValue || envValue.trim() === '') {
        return [];
    }

    try {
        // 一些云平台在注入多行环境变量时会自动在外层包一层引号（例如 '"[ {...} ]"'），
        // 这里做一次健壮性处理，去掉首尾成对的引号，避免 JSON.parse 报错。
        let modelsJson = envValue.trim();
        const firstChar = modelsJson[0];
        const lastChar = modelsJson[modelsJson.length - 1];
        if (
            (firstChar === '"' && lastChar === '"') ||
            (firstChar === "'" && lastChar === "'")
        ) {
            modelsJson = modelsJson.slice(1, -1).trim();
        }
        ...
        return models.map((model) => ({
            ...
            supportsToolCalls: model.supportsToolCalls ?? true, // 默认支持工具调用
...
export async function fetchAvailableModels() {
    ...
    // 检查是否是 DP Tech API
    const isDPTech = baseUrl.includes('openapi.dp.tech');
    
    // DP Tech API 不支持 /models 端点，需要手动配置 SYSTEM_MODELS
    if (isDPTech) {
        console.warn("DP Tech API 不支持 /models 端点，请在环境变量中手动配置 SYSTEM_MODELS");
        return [];
    }
    ...
    try {
        ...
        return models
            ...
            .map((model) => ({
                ...
                isStreaming: true,
                supportsToolCalls: true, // 动态获取的模型默认支持工具调用
            }))
            .filter((model) => model.id && model.id.trim().length > 0);
    } catch (error) {
...
export function getSystemModelConfig(modelId) {
    ...
    if (staticModels.length === 0) {
        ...
        return {
            ...
            label: modelId,
            isStreaming: true,
            supportsToolCalls: true, // 动态模型默认支持工具调用
            baseUrl: baseUrl.trim().replace(/\/$/, ''),
            apiKey: apiKey.trim(),
        };
    }
...
export function resolveSystemModel(modelId) {
    ...
    // 检查是否是 DP Tech API（使用 accessKey 认证方式）
    const isDPTech = config.baseUrl.includes('openapi.dp.tech');
    
    let clientConfig;
    if (isDPTech) {
        // DP Tech API 使用 accessKey 头部而不是 Authorization Bearer
        // 需要通过自定义 fetch 函数来添加 accessKey 头部
        const originalFetch = globalThis.fetch;
        clientConfig = {
            apiKey: 'dp-tech-placeholder', // SDK 需要这个参数，但不会使用
            baseURL: config.baseUrl,
            fetch: async (url, options) => {
                // 自定义 fetch：添加 accessKey 头部，移除 Authorization 头部
                const customHeaders = {
                    ...options?.headers,
                    'accessKey': config.apiKey, // DP Tech 的自定义认证头部
                };
                
                // 移除 Authorization 头部（AI SDK 默认会添加）
        ...
        // 标准 OpenAI 兼容 API（使用 Authorization: Bearer）
        clientConfig = {
    ...
    return {
        ...
        supportsToolCalls: config.supportsToolCalls ?? true, // 默认支持工具调用
    };
}
