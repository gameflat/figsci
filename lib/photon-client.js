 ...
 * 获取当前收费模式
 * 
 * @returns {'fixed' | 'token' | 'mixed'} 收费模式
 */
export function getChargeMode() {
  return process.env.NEXT_PUBLIC_BOHRIUM_CHARGE_MODE || 'fixed';
}

/**
 * 预扣费结果类型
 * @typedef {Object} PreChargeResult
 * @property {boolean} success - 是否成功
 * @property {string} message - 消息
 * @property {boolean} needsCharge - 是否需要扣费
 * @property {number} [eventValue] - 扣费金额
 * @property {number} [bizNo] - 业务编号
 ...
 * 调用预扣费接口
 * 
 * 在 mixed 收费模式下，发送消息前调用此函数预扣固定费用。
 * 如果余额不足或扣费失败，应阻止消息发送。
 * 
 * @returns {Promise<PreChargeResult>} 预扣费结果
 */
export async function preChargePhoton() {
  try {
    const response = await fetch('/api/photon/pre-charge', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      }
    ...
    // 即使 HTTP 状态码不是 200，也返回解析后的数据
    // 因为后端会在响应体中包含 success 字段
    return {
      success: data.success ?? false,
      message: data.message || data.error || '预扣费失败',
      needsCharge: data.needsCharge ?? true,
      eventValue: data.eventValue,
      bizNo: data.bizNo,
      chargeMode: data.chargeMode,
      isInsufficientBalance: data.isInsufficientBalance ?? false
    };
  } catch (error) {
    console.error('预扣费请求失败：', error);
    return {
      success: false,
      message: error instanceof Error ? error.message : '预扣费请求失败',
      needsCharge: true,
 ...
 * 获取固定费用金额（用于 UI 显示）
 * 
 * @returns {number} 每次请求的固定费用
 */
export function getFixedChargeAmount() {
  return parseInt(process.env.NEXT_PUBLIC_BOHRIUM_CHARGE_PER_REQUEST || '1');
}

/**
 * 获取每 1000 token 的费用（用于 UI 显示）
 * 
 * @returns {number} 每 1000 token 的费用
 */
export function getTokenChargeRate() {
  return parseFloat(process.env.NEXT_PUBLIC_BOHRIUM_CHARGE_PER_1K_TOKEN || '1');
}
